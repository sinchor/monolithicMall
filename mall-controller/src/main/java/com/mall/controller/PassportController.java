...
public class PassportController {
    ...
    public MallJSONResult usernameExists(@RequestParam String username) {
        if (StringUtils.isEmpty(username)) {
            return MallJSONResult.errorMsg(I18nMessage.message("passport.error.usernameNull"));
        }
        boolean nameExists = userService.queryUserNameExists(username);
        if (nameExists) {
            return MallJSONResult.errorMsg(I18nMessage.message("passport.error.usernameExists"));
        }
    ...
    public MallJSONResult register(@RequestBody UserBo userBo) {
        ...
        if (StringUtils.isBlank(username) || StringUtils.isBlank(password) || StringUtils.isBlank(confirmPassword)) {
            return MallJSONResult.errorMsg(I18nMessage.message("passport.error.un_psw_null"));
        }
        ...
        if (password.length() < 6) {
            return MallJSONResult.errorMsg(I18nMessage.message("passport.error.shortPswLength"));
        }
        // 3. 判断两次密码是否一致
        if (! password.equals(confirmPassword)) {
            return MallJSONResult.errorMsg(I18nMessage.message("passport.error.pswMismatched"));
        }
    ...
    public MallJSONResult login(@RequestBody UserBo userBo) {
        ...
        if (user == null) {
            return MallJSONResult.errorMsg(I18nMessage.message("passport.error.un_psw_error"));
        }
